...
- **ポスター**（`.stage`）＝ 紙のざらつき（SVGノイズを overlay）＋ 斜めのハーフトーン帯 ＋ 傾けた金の板 ＋
  巨大なローマ字（1行目は実線、2行目は輪郭を右へずらす）＋ 切り抜き写真。板と文字はわざと傾け、端は切らせる
  （整えすぎると安っぽくなる）。写真が無い選手はエンブレムを薄く置き「PHOTO COMING SOON」の札
- **名鑑カード**（`.mcard`）＝ 3:4 の小さなポスター。金の板（-2.5°）に名前とかな、左上にポジション、右下に生年月日
- **ローマ字は `data-photo` の前半**（`iwamura-naoki` → IWAMURA）から JS が作る。HTML に足すものは無い
